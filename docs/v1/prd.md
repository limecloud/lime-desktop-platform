...
Lime 组织已经进入多桌面 App 并行阶段，`content-studio`、`zhongcao` 以及后续 OEM App 都会复用同一批基础能力。
...
| 模块 | 平台职责 | 业务 App 不负责 |
| --- | --- | --- |
| Agent App Host Runtime | App 安装、校验、投影、readiness、Host Bridge、能力路由 | 自己实现宿主协议 |
| 应用中心 | 拉取目录、展示状态、安装、更新、启用、禁用 | 自己维护应用目录 |
...
| `lime-desktop-platform` | 本地宿主、应用中心、模型设置、OEM 壳层、会话、桥接、平台配置 | 业务工作流本身 |
...
App 启动后只能通过宿主桥接调用平台能力，不能直接访问宿主内部实现。
...
flowchart TD
  ...
  Catalog --> Installed{本地是否已安装?}

  Installed -- 否 --> Download[下载应用包]
  Installed -- 是 --> Verify[校验包与 manifest]
  Download --> Verify
...
- `content-studio` 和 `zhongcao` 可以共用同一套应用中心、会话、模型设置和 OEM 壳层能力。
...
- 平台底座是整个 App 线的基础。
- `content-studio`、`zhongcao`、后续 OEM App 都是底座消费者。
