...
- 模型页复刻桌面参考设置结构：左侧为“启用的模型”二级列表，默认来自平台 `ModelSettings.providers`；右侧为当前 provider 配置卡，包含 Provider 名称、Base URL、API 格式、Responses API、API 密钥配置状态、模型优先级、添加模型、上移 / 下移 / 移除模型和测试连接。
