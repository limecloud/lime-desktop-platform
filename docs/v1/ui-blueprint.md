...
- `PlatformAccountEntry` 与 `PlatformSettingsDialog` 共享 `PlatformSettingsThemeTokens` 主题契约。宿主 App 可以通过 `theme`、`className` 和 `--lime-platform-*` CSS 变量对齐字体、颜色、圆角、边框和阴影；禁止 fork `.lime-settings-*` DOM 或把平台设置页复制到业务仓库。
- 左侧设置分类必须完整可进入：通用、个性化、主题、每日回顾、模型、使用统计、语音模型、开放网关、机器人对话、搜索服务、网络、数据、账号和关于。当前未接真实保存的分类展示平台投影页，不在 Product App 内做私有设置事实源。
- 主题页复刻桌面参考设置结构：包含外观模式、颜色主题、对话字体大小和衬线体开关；平台 React 包通过 `onPreviewPlatformSettings` 立即预览，通过 `onSavePlatformSettings` 交给宿主统一保存，并由 Host Snapshot 的 `theme` / `appearance` 驱动 Product App 壳层。
- 模型页复刻桌面参考设置结构：左侧为“已配置 Provider”列表，默认来自平台 `ModelSettings.providers`；右侧为当前 provider 配置卡，包含 Provider 名称、Base URL、API 格式、Responses API、API 密钥配置状态、模型优先级、添加模型、上移 / 下移 / 移除模型和测试连接。
- “添加模型”不是详情页下方的常驻目录，也不是平台内置品牌目录。点击左侧 `+` 或“添加模型”后创建一个空白自定义 Provider；平台不预置推荐服务、国内服务、聚合平台、海外平台或本地模型 tabs，不自动写入 `default-model`。用户必须显式填写 Provider metadata 和至少一个模型 ID 后保存。
...
- 搜索服务页当前只维护 UI 草稿状态，真实 API Key 加密保存、Google CSE CX、秘塔兼容端点、provider 健康检查、WebSearch 路由和失败自动回退必须由平台 host-core search action handler 或 App Server provider/key 控制面接入。Product App 只能挂载该页面，不保存搜索凭证、搜索优先级或 WebSearch 路由事实源。
...
- 网络页当前只维护 UI 草稿状态，真实系统代理检测、代理配置保存、`HTTP_PROXY` / `HTTPS_PROXY` 环境变量注入、App Server sidecar 代理隔离和证书 / 网关策略必须由平台 host-core settings / network action handler 接入。Product App 只能挂载该页面，不保存代理配置、不覆盖系统代理、不直接写 AI 子进程环境变量。
