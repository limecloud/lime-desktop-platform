...
- `src/main/services/credentialBroker.ts` 和 `src/main/services/modelSettingsCredentials.ts` 提供最小本地 Credential Broker：`settings.saveModel` 临时输入中的 provider 凭证写入 broker，普通 `model-settings.json` 只保留 `apiKeyConfigured` 投影。
...
- Credential Broker 当前完成最小本地实现：`settings.saveModel` 输入中的临时 API Key / OAuth 凭证会写入 broker，普通 `ModelSettings` JSON 只保留 `apiKeyConfigured`，`CredentialBroker.resolveModelProviderCredential(...)` 只在受控设置同步边界解析凭证；Desktop Host 会用 `modelProvider/list/read/create/update` 同步 provider metadata，并用 `modelProviderKey/create` 写入 App Server provider key。diagnostics / runtimeContext 已输出非敏感 credential readiness、storage kind、keychain-backed 状态、rotation 状态、`broker-reference-only` / `app-server-provider-ready` handoff 状态和 App Server provider id。OS keychain、OAuth token 轮换、过期刷新和生产级 credential injection 仍未完成。
