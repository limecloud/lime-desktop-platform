...
| [`agent-runtime-strategy.md`](./agent-runtime-strategy.md) | Lime App Server JSON-RPC / RuntimeCore 的 Agent Runtime 接入策略。 |
...
4. 再定 Agent Runtime：current 接入 Lime App Server JSON-RPC / RuntimeCore；Pi agent 和 Claude SDK 不再作为 current/proposed 后端路线。
...
- Agent Runtime 执行事实归 Lime App Server JSON-RPC / RuntimeCore，不与桌面壳混淆；桌面壳只做 host-mediated bridge。
...
- Pi agent、Claude SDK、MCP session tools 不属于平台 current/proposed 后端路线；Product App 不直接 import provider SDK，也不复制 provider session manager。
- 平台基础设置由 `lime-desktop-platform` 实现；业务 App 只允许通过 Product Settings extension 暴露独特设置，并写入独立 `product-settings` namespace，且不能保存凭证、token、API Key 或 OAuth 数据；业务草稿、历史和工作流状态走 `lime.storage`，不能塞进设置 JSON。
